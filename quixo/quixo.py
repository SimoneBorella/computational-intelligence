...
def reward_1(res, current_player):
    reward = 0
    ...
    for ep in range(episode_num):
        ...
        while not done and step < max_episode_steps:
            if current_player == 0:
                ...
                if done:
                    # reward = reward_1(res, current_player)
...
class QuixoIsSolvedAgent(Agent):
    def __init__(self, player_id) -> None:
        super().__init__(player_id)
        self.outcomes = Outcomes()
        self.outcomes_rev = Outcomes()
        self.outcomes_next_rev = Outcomes()
    ...
    def choose_action(self, state):
        ...
        try:
            ...
            cx, co = get_class(encoded_state)
            
            self.outcomes.read(cx, co)
            self.outcomes_rev.read(co, cx)
            if co + cx < N*N:
                self.outcomes_next_rev.read(co, cx+1)
            
            draw_actions = []
            win_actions = []
            for action in available_actions:
                game = QuixoGame(state.flatten())
                child_state, _, _ = game.step(action, self.player_id)
                encoded_child_state = swap(encode_state(child_state.reshape((N, N))))
                ccx, cco = get_class(encoded_child_state)

                # Child belonging to the same class
                if ccx == co and cco == cx:
                    if self.outcomes_rev.is_loss(encoded_child_state):
                        win_actions.append(action)
                    elif self.outcomes_rev.is_draw(encoded_child_state):
                        draw_actions.append(action)
                    
                # Child belonging to next class
                else:
                    if self.outcomes_next_rev.is_loss(encoded_child_state):
                        win_actions.append(action)
                    elif self.outcomes_next_rev.is_draw(encoded_child_state):
                        draw_actions.append(action)

            if len(win_actions) > 0:
                # print("WIN ACTION TAKEN")
                return random.choice(win_actions)
            if len(draw_actions) > 0:
                # print("DRAW ACTION TAKEN")
                return random.choice(draw_actions)
            
            # print("LOSE ACTION TAKEN")
            return random.choice(available_actions)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Quixo game agent testing.')
    parser.add_argument('test_agent', help='Agent to test (MiniMaxAgent, DeepQLearningAgent, QuixoIsSolvedAgent)')
    parser.add_argument('-n', '--nsize', type=int, default=4, help='Quixo board size (N)')
    parser.add_argument('-e', '--episodes', type=int, default=500, help='Testing episodes')
    parser.add_argument('-m', '--maxepisodes', type=int, default=200, help='Max testing episodes')
    ...
    if not render and test_agent == "MiniMaxAgent":
        print("MiniMaxAgent testing:")

        print("MiniMaxAgent(2) vs RandomAgent (init_player=0):")
        test(
            agent=MiniMaxAgent(player_id=0, depth=2),
            opponent_agents=(RandomAgent(player_id=1),),
            episode_num=episodes,
            max_episode_steps=maxepisodes,
            verbose=verbose,
            init_player=0
        )
        print("-"*40)

        print("MiniMaxAgent(2) vs RandomAgent (init_player=1):")
        test(
            agent=MiniMaxAgent(player_id=0, depth=2),
            opponent_agents=(RandomAgent(player_id=1),),
            episode_num=episodes,
        ...
        print("MiniMaxAgent(2) vs MiniMaxAgent(2) (init_player=0):")
        test(
            agent=MiniMaxAgent(player_id=0, depth=2),
        ...
        print("MiniMaxAgent(2) vs MiniMaxAgent(2) (init_player=1):")
        test(
            agent=MiniMaxAgent(player_id=0, depth=2),
            opponent_agents=(MiniMaxAgent(player_id=1, depth=2),),
            episode_num=episodes,
        ...
        print("MiniMaxAgent(3) vs MiniMaxAgent(2) (init_player=0):")
        test(
            agent=MiniMaxAgent(player_id=0, depth=3),
            opponent_agents=(MiniMaxAgent(player_id=1, depth=2),),
            episode_num=episodes,
            max_episode_steps=maxepisodes,
            verbose=verbose,
        ...
        print("MiniMaxAgent(3) vs MiniMaxAgent(2) (init_player=1):")
        test(
            agent=MiniMaxAgent(player_id=0, depth=3),
            opponent_agents=(MiniMaxAgent(player_id=1, depth=2),),
            episode_num=episodes,
    ...
    if not render and test_agent == "DeepQLearningAgent":
        print("DeepQLearningAgent testing:")

        agent = DeepQLearningAgent(player_id=0, gamma=0.98, lr=0.01, state_dim=N*N, batch_size=64, actions_dim=N*4)
        if N == 4:
            model_path = "./models/model_4x4.pt"
        elif N == 5:
            model_path = "./models/model_5x5.pt"
        
        agent.load_model(model_path)

        print("DeepQLearningAgent vs RandomAgent (init_player=0):")
        test(
            agent=agent,
            opponent_agents=(RandomAgent(player_id=1), ),
            episode_num=episodes,
        ...
        print("DeepQLearningAgent vs RandomAgent (init_player=1):")
        test(
            agent=agent,
            opponent_agents=(RandomAgent(player_id=1), ),
            episode_num=episodes,
        ...
        print("DeepQLearningAgent vs MiniMaxAgent(2) (init_player=0):")
        test(
            agent=agent,
            opponent_agents=(MiniMaxAgent(player_id=1, depth=2), ),
            episode_num=episodes,
        ...
        print("DeepQLearningAgent vs MiniMaxAgent(2) (init_player=1):")
        test(
            agent=agent,
            opponent_agents=(MiniMaxAgent(player_id=1, depth=2), ),
            episode_num=episodes,
    ...
    if not render and test_agent == "QuixoIsSolvedAgent":
        print("QuixoIsSolvedAgent testing:")

        print("QuixoIsSolvedAgent vs MiniMaxAgent(2) (init_player=0):")
        test(
            agent=QuixoIsSolvedAgent(player_id=0),
            opponent_agents=(MiniMaxAgent(player_id=1, depth=2), ),
            episode_num=episodes,
            max_episode_steps=maxepisodes,
            verbose=verbose,
        ...
        print("QuixoIsSolvedAgent vs MiniMaxAgent(2) (init_player=1):")
        test(
            agent=QuixoIsSolvedAgent(player_id=0),
            opponent_agents=(MiniMaxAgent(player_id=1, depth=2), ),
            episode_num=episodes,
        ...
        print("QuixoIsSolvedAgent vs MiniMaxAgent(3) (init_player=0):")
        test(
            agent=QuixoIsSolvedAgent(player_id=0),
            opponent_agents=(MiniMaxAgent(player_id=1, depth=3), ),
            episode_num=episodes,
            max_episode_steps=maxepisodes,
            verbose=verbose,
            init_player=0
        ...
        print("QuixoIsSolvedAgent vs MiniMaxAgent(3) (init_player=1):")
        test(
            agent=QuixoIsSolvedAgent(player_id=0),
            opponent_agents=(MiniMaxAgent(player_id=1, depth=3), ),
            episode_num=episodes,
        ...
        print("QuixoIsSolvedAgent vs QuixoIsSolvedAgent (init_player=0):")
        test(
            agent=QuixoIsSolvedAgent(player_id=0),
            opponent_agents=(QuixoIsSolvedAgent(player_id=1), ),
            episode_num=episodes,
            max_episode_steps=maxepisodes,
            verbose=verbose,
            init_player=0
        ...
        print("QuixoIsSolvedAgent vs QuixoIsSolvedAgent (init_player=1):")
        test(
            agent=QuixoIsSolvedAgent(player_id=0),
            opponent_agents=(QuixoIsSolvedAgent(player_id=1), ),
            episode_num=episodes,
    ...
    if render and test_agent == "MiniMaxAgent":
        print("MiniMaxAgent(2) vs RandomAgent:")
        test(
            agent=MiniMaxAgent(player_id=0, depth=2),
            opponent_agents=(RandomAgent(player_id=1), ),
            episode_num=1,
        ...
        print("MiniMaxAgent(2) vs MiniMaxAgent(2):")
        test(
            agent=MiniMaxAgent(player_id=0, depth=2),
            opponent_agents=(MiniMaxAgent(player_id=0, depth=2), ),
            episode_num=1,
            max_episode_steps=maxepisodes,
            verbose=verbose,
            render=render
    ...
    if render and test_agent == "DeepQLearningAgent":
        print("DeepQLearningAgent testing:")
        ...
        print("DeepQLearningAgent vs RandomAgent:")
        test(
            agent=agent,
            opponent_agents=(RandomAgent(player_id=1), ),
            episode_num=episodes,
        ...
        print("DeepQLearningAgent vs MiniMaxAgent(2):")
        test(
            agent=agent,
            opponent_agents=(MiniMaxAgent(player_id=1, depth=2), ),
            episode_num=episodes,
    ...
    if render and test_agent == "QuixoIsSolvedAgent":
        print("QuixoIsSolvedAgent vs MiniMaxAgent(2):")
        test(
            agent=QuixoIsSolvedAgent(player_id=0),
            opponent_agents=(MiniMaxAgent(player_id=1, depth=2), ),
            episode_num=1,
        ...
        print("QuixoIsSolvedAgent vs QuixoIsSolvedAgent:")
        test(
            agent=QuixoIsSolvedAgent(player_id=0),
            opponent_agents=(QuixoIsSolvedAgent(player_id=1), ),
            episode_num=1,
