...
int get_state_class_id(state_t state)
{
    state_t mask = (1 << (N * N)) - 1;
    state_t o = state & mask;
    state_t x = ((state & (mask << SHIFT)) >> SHIFT) & mask;
...
{
public:
    static void dispositions(int pos, state_t sol_x, int mark_x[2], state_t sol_o, std::queue<state_t> &states_queue, std::condition_variable &cvar, std::mutex &mutex, bool &next, bool &stop)
    {
        if (pos >= N * N)
        {
    ...
    static void state_dispositions(int pos, int cx, int co, state_t sol_o, int mark_o[2], std::queue<state_t> &states_queue, std::condition_variable &cvar, std::mutex &mutex, bool &next, bool &stop)
    {
        if (pos >= N * N)
        {
            state_t sol_x = 0;
    ...
    static void compute_class_states(int cx, int co, std::queue<state_t> &states_queue, std::condition_variable &cvar, std::mutex &mutex, bool &next, bool &stop)
    {
        state_t sol_o = 0;
                        ...
                        if(outcomes.is_draw(state))
                        {
                            ...
                            for(auto m : get_available_moves(state, 0))
                            {
                                ...
                                if(cx != co)
                                {
                                    if(ccx==co && cco==cx)
                                    {
                                        if(!outcomes_rev.is_win(child_state))
                                            loss = false;
                                    ...
                                    {
                                        if(!outcomes_next_rev.is_win(child_state))
                                            loss = false;
                                ...
                                else
                                {
                                    if(ccx==co && cco==cx)
                                    {
                                        if(!outcomes.is_win(child_state))
                                            loss = false;
                    ...
                    while (iterator.has_next())
                    {
                        ...
                        if(outcomes_rev.is_draw(state))
                        {
                            ...
                            for(auto m : get_available_moves(state, 0))
                            {
                                ...
                                {
                                    if(!outcomes_next.is_win(child_state))
                                        loss = false;
